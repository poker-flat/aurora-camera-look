 ...
 *
 * Parameters:
 * x - {float} X coordinate (longitude in decimal degrees)
 * y - {float} Y coordinate (latitude in decimal degrees)
 * z - {float} Z coordinate (altitude in kilometers)
 */
...
Point.KM2MI = 0.621371;
Point.KM2NMI = 0.539967;
Point.MI2KM = 1.60934;
Point.MI2NMI = 0.868976;
Point.NMI2KM = 1.852;
Point.NMI2MI = 1.15078;
...
Point.prototype.geoDistanceTo = function(point, units) {
    ...
    if (units && units.toLowerCase() == 'km') { radius = Point.EARTH_RADIUS_KM; alt_conv = 1; }
    else if (units && units.toLowerCase() == 'mi') { radius = Point.EARTH_RADIUS_MI; alt_conv = Point.KM2MI; }
    else if (units && units.toLowerCase() == 'nmi') { radius = Point.EARTH_RADIUS_NMI; alt_conv = Point.KM2NMI; }
    else { units = 'km'; radius = Point.EARTH_RADIUS_KM; alt_conv = 1; }
    ...
    if (this.geoIsValid() && point.geoIsValid()) {
        ...
        var dist = 2 * Math.asin(c) * (radius + (this.z * alt_conv));
 ...
 * http://williams.best.vwh.net/avform.htm
 */
Point.prototype.geoDistanceTo3D = function(point, units) {
    var x = [];
    var y = [];
    var radius = 0;
    var alt_conv = 1;
    ...
    if (this.geoIsValid() && point.geoIsValid()) {
        ...
        var dist = 2 * Math.asin(c) * (radius + ((this.z + point.z)/2.0 * alt_conv));
        ...
        if (this.z == point.z) {
            ...
            var alt = 0;
            if (this.z > point.z) {
                alt = (this.z - point.z) * alt_conv;
            } else {
                alt = (point.z - this.z) * alt_conv;
            }
            return Math.sqrt(dist*dist + alt*alt);
        }
    } else {
        return null;
    }
...
/**
 * Method: geoElevationTo
 */
Point.prototype.geoElevationTo = function(point) {
    if (this.geoIsValid() && point.geoIsValid()) {
        var dist = this.geoDistanceTo3D(point);
        var elev = Math.asin((point.z - this.z)/dist) * Point.RAD2DEG;
        return elev;
    }
    else {
        return null;
...
var rad_conv = 1;
    ...
    // Calculate the radius of the earth used
    if (units && units.toLowerCase() == 'km') { radius = Point.EARTH_RADIUS_KM; rad_conv = 1; }
    else if (units && units.toLowerCase() == 'mi') { radius = Point.EARTH_RADIUS_MI; rad_conv = Point.KM2MI; }
    else if (units && units.toLowerCase() == 'nmi') { radius = Point.EARTH_RADIUS_NMI; rad_conv = Point.NMI2MI; }
    else { radius = Point.EARTH_RADIUS_KM; rad_conv = 1; }
    ...
    var c = distance / (radius + (this.z * rad_conv));
